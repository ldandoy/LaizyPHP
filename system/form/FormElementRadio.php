...
class FormElementRadio extends FormElement
{
	...
	public function getHtml()
	{
		...
		{
			$html .=
					'<input value="1"'.$checked.' disabled="disabled" type="radio" class="form-control'.$class.'" />'.
					'<input id="'.$this->id.'" name="'.$this->id.'" type="hidden" value="'.$this->value.'" />';
		}
		else
		{
			$html .=
					'<input id="'.$this->id.'" name="'.$this->id.'" value="1"'.$checked.' type="radio" class="form-control'.$class.'" />';
		}
		$html .=
				'</div>'.
				$this->getErrorHtml().
